...
import { DetailMouseInfo } from './DetailMouseInfo';
...
const DetailData = ({ data, productId, mutate }) => {
	...
	const [details, setDetails] = useState(
		data?.details || DetailMouseInfo.details
	);
	const [loading, setLoading] = useState(false);
	...
	const handleSubmitUpdateDetail = (e) => {
		...
		if (!data) {
			postData(
				'products/mouses/' + productId + '/details',
				{
					guaranteePeriod: 10,
					includedAccessories: [
						'Sách hướng dẫn',
			...
			updateData(
				'products/mouses/' + productId + '/details/' + data._id,
				{
					guaranteePeriod: 10,
					includedAccessories: [
						'Sách hướng dẫn',
...
const VariantItem = ({ data, productId, mutate }) => {
	...
	const handleSubmitUpdateVariant = (e) => {
		...
		patchFormData(
			'products/mouses/' + productId + '/variants/' + data._id,
			formData,
			{
				headers: {
					Authorization: 'Bearer ' + token,
	...
	const handleDeleteVariant = (e) => {
		...
		deleteData('products/mouses/' + productId + '/variants/' + data._id, {
...
const AddVariant = ({ productId, mutate }) => {
	...
	const handleSubmitAddVariant = (e) => {
		...
		postFormData('products/mouses/' + productId + '/variants/', formData, {
			headers: {
				Authorization: 'Bearer ' + token,
				'Content-Type': 'multipart/form-data;',
			},
...
const EditMouse = () => {
	...
	const { data: variantList, mutate: mutateVariantList } = useSWR(
		'products/mouses/' + productId + '/variants',
		getData,
		SWRconfig
	);
