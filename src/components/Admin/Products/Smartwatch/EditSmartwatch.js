...
import { DetailSmartwatchInfo } from './DetailSmartwatchInfo';
...
const GeneralData = ({ data, productId, mutate }) => {
	...
	const handleUpdateGeneralData = (e) => {
		...
		setLoading(true);
		updateData(
			'products/smartwatches/' + productId,
...
const DetailData = ({ data, productId, mutate }) => {
	...
	const [screen, setScreen] = useState(
		data?.screen || DetailSmartwatchInfo.screen
	);
	const [design, setDesign] = useState(
		data?.design || DetailSmartwatchInfo.design
	);
	const [utility, setUtility] = useState(
		data?.utility || DetailSmartwatchInfo.utility
	);
	const [battery, setBattery] = useState(
		data?.battery || DetailSmartwatchInfo.battery
	);
	const [configurationAndConnection, setConfigurationAndConnection] =
		useState(
			data?.configurationAndConnection ||
				DetailSmartwatchInfo.configurationAndConnection
		);
	const [additionalInformation, setAdditionalInformation] = useState(
		data?.additionalInformation ||
			DetailSmartwatchInfo.additionalInformation
	);
	const [loading, setLoading] = useState(false);
	...
	const handleSubmitUpdateDetail = (e) => {
		...
		if (!data) {
			postData(
				'products/smartwatches/' + productId + '/details',
				{
					guaranteePeriod: 10,
					includedAccessories: [
						'Sách hướng dẫn',
					...
					utility: utility,
					battery: battery,
					configurationAndConnection: configurationAndConnection,
			...
			updateData(
				'products/smartwatches/' + productId + '/details/' + data._id,
				{
					guaranteePeriod: 10,
					includedAccessories: [
						'Sách hướng dẫn',
		...
		<div className="w-full bg-white rounded-md border-[1px] border-solid border-gray-300 shadow-sm shadow-gray-400">
			<form className="grid grid-cols-2">
				...
				<div className="mb-4">
					...
					<DetailItem list={design} handleChange={setDesign} />
				</div>
				...
				<div className="mb-4">
					...
					<DetailItem list={battery} handleChange={setBattery} />
				</div>
				...
				<div className="mb-4">
					...
					<DetailItem
						list={configurationAndConnection}
						handleChange={setConfigurationAndConnection}
					/>
				</div>
...
const VariantItem = ({ data, productId, mutate }) => {
	...
	const handleSubmitUpdateVariant = (e) => {
		...
		patchFormData(
			'products/smartwatches/' + productId + '/variants/' + data._id,
			formData,
			{
				headers: {
					Authorization: 'Bearer ' + token,
...
const AddVariant = ({ productId, mutate }) => {
	...
	const handleSubmitAddVariant = (e) => {
		...
		postFormData(
			'products/smartwatches/' + productId + '/variants/',
			formData,
			{
				headers: {
					Authorization: 'Bearer ' + token,
