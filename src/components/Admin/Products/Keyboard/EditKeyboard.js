...
import { DetailKeyboardInfo } from './DetailKeyboardInfo';
...
const DetailData = ({ data, productId, mutate }) => {
	...
	const [details, setDetails] = useState(
		data?.details || DetailKeyboardInfo.details
	);
	const [loading, setLoading] = useState(false);
	...
	const handleSubmitUpdateDetail = (e) => {
		...
		if (!data) {
			postData(
				'products/keyboards/' + productId + '/details',
				{
					guaranteePeriod: 10,
					includedAccessories: [
						'Sách hướng dẫn',
			...
			updateData(
				'products/keyboards/' + productId + '/details/' + data._id,
				{
					guaranteePeriod: 10,
					includedAccessories: [
						'Sách hướng dẫn',
...
const VariantItem = ({ data, productId, mutate }) => {
	...
	const handleSubmitUpdateVariant = (e) => {
		...
		patchFormData(
			'products/keyboards/' + productId + '/variants/' + data._id,
			formData,
			{
				headers: {
					Authorization: 'Bearer ' + token,
...
const AddVariant = ({ productId, mutate }) => {
	...
	const handleSubmitAddVariant = (e) => {
		...
		postFormData(
			'products/keyboards/' + productId + '/variants/',
			formData,
			{
				headers: {
					Authorization: 'Bearer ' + token,
