...
import { DetailLaptopInfo } from './DetailLaptopInfo';
...
const DetailData = ({ data, productId, mutate }) => {
	...
	const [processor, setProcessor] = useState(
		data?.processor || DetailLaptopInfo.processor
	);
	const [ram, setRam] = useState(
		data?.ramMemoryAndHardDrive || DetailLaptopInfo.ramMemoryAndHardDrive
	);
	const [screen, setScreen] = useState(
		data?.screen || DetailLaptopInfo.screen
	);
	const [graphicsAndAudio, setGraphicsAndAudio] = useState(
		data?.graphicsAndAudio || DetailLaptopInfo.graphicsAndAudio
	);
	const [connection, setConnection] = useState(
		data?.connectionPortAndExpansionFeature ||
			DetailLaptopInfo.connectionPortAndExpansionFeature
	);
	const [size, setSize] = useState(
		data?.sizeAndWeight || DetailLaptopInfo.sizeAndWeight
	);
	const [additionalInformation, setAdditionalInformation] = useState(
		data?.additionalInformation || DetailLaptopInfo.additionalInformation
	);
	const [loading, setLoading] = useState(false);
		...
		if (!data) {
			postData(
				'products/laptops/' + productId + '/details',
				{
					...
					processor: processor,
					ramMemoryAndHardDrive: ram,
					screen: screen,
					graphicsAndAudio: graphicsAndAudio,
					connectionPortAndExpansionFeature: connection,
					sizeAndWeight: size,
		...
		<div className="w-full bg-white rounded-md border-[1px] border-solid border-gray-300 shadow-sm shadow-gray-400">
			<form className="grid grid-cols-2">
				<div className="mb-4">
					...
					<DetailItem list={processor} handleChange={setProcessor} />
				</div>
				...
				<div className="mb-4">
					...
					<DetailItem list={ram} handleChange={setRam} />
				</div>
				...
				<div className="mb-4">
					...
					<DetailItem
						list={graphicsAndAudio}
						handleChange={setGraphicsAndAudio}
					/>
				</div>
				...
				<div className="mb-4">
					...
					<DetailItem list={size} handleChange={setSize} />
				</div>
	...
	const handleSubmitUpdateVariant = (e) => {
		...
		patchFormData(
			...
			.then((res) => {
				...
				// getData('products/laptops/' + productId + '/variants').then(
				// 	(res) => {
				// 		mutate(res);
				// 	}
				// );
		...
		<AdminLayout>
			<div className="text-main">
				...
				<div className="w-full flex flex-col gap-4">
					...
					{detailLaptopInfo && (
						<HighLightImages
							data={detailLaptopInfo?.highlightedImages}
							productId={productId}
							detailId={detailLaptopInfo?._id}
							mutate={mutateDetailData}
						/>
					)}
